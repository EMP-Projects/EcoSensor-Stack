...
aws_topic_arc=$(aws ssm get-parameter --with-decryption --region us-east-1 --name "ECOSENSOR_AWS_TOPIC_ARN" --query "Parameter.Value" --output text) 
aws_bucket_name_data=$(aws ssm get-parameter --with-decryption --region us-east-1 --name "ECOSENSOR_AWS_BUCKET_NAME_DATA" --query "Parameter.Value" --output text) 
github_token=$(aws ssm get-parameter --with-decryption --region us-east-1 --name "GITHUB_TOKEN" --query "Parameter.Value" --output text) 
